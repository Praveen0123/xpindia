...
@Component({
  selector: 'app-add-new-patient-order',
  templateUrl: './add-new-patient-order.component.html',
  styleUrls: ['./add-new-patient-order.component.scss']
})
export class AddNewPatientOrderComponent implements OnInit {
  ...
  constructor(public alertService:AlertService,public dialogRef: MatDialogRef<AddNewPatientOrderComponent>, public fb: FormBuilder,@Inject(MAT_DIALOG_DATA) public order: any) { 
  ...
  saveOrder() {
    this.alertService.createAlert('Case added successfully.', 1);
    this.dialogRef.close();
  }
